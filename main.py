...
def train(args):
    ...
    #  把 CLIP 默认的预处理改成我们自己要的尺寸
    target_transform = transforms.Compose([
        transforms.Resize(size=clip_transform.transforms[0].size, interpolation=transforms.InterpolationMode.BICUBIC),
        transforms.ToTensor()
    ])  # 再构造一个给“目标图像”用的 transform
    clip_model.eval()
    
    for param in clip_model.parameters():  # 把 CLIP 切换到推理模式并冻结参数
        param.requires_grad_(False)

    clip_model = clip_model.to(device)
    clip_model.insert(args=args, tokenizer=tokenize, device=device)  # 给 CLIP 模型“打补丁”插入自定义模块
    ...
    if args.weight is not None:  # 加载预训练权重
