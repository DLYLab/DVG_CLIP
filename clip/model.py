...
from clip.adaptor_mlp import LinearAdaptor
from clip.ChannelPool import GroupedChannelPool
from clip.chuli import CLSTokenProcessor
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.transformer = Transformer(width, layers, heads,)  # 自定义的 Transformer 编码器（未给出定义），输入是 patch 嵌入序列
        self.attn = None
    ...
    def forward(self, x: torch.Tensor, feature_layers=[24], visual_prompt=None):
        ...
        if side != new_side:  # 如果输入图像尺寸与训练时不一致，会对位置编码进行双线性插值，适配新的 patch 数量。
        ...
        if visual_prompt is not None:  # 将视觉提示拼接到序列末尾，用于 prompt tuning 或适配任务。
            x = torch.cat([x, visual_prompt[:1].repeat(x.size(0), 1, 1)], dim=1)
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)  # NLD -> LND
        out = self.transformer(x, feature_layers)
        for i, o in enumerate(out):  # 如果用了 visual prompt，会把它从输出中去掉。
...
class CLIP(nn.Module):
    ...
    def insert(self, args, tokenizer, device):  # “可学习的文本 prompt 向量 + 轻量 adaptor + 高斯核 + 内存库占位符”一次性插到 CLIP 模型里，
        self.normal_cls_prompt = f'without defect.'
        self.anomaly_cls_prompt = f'with defect.'  # 两条固定模板
        ...
        # self.adaptor_cls = LinearAdaptor(inplanes=1024, outplanes=786).to(device)
        # self.augment = GroupedChannelPool(num_layers=24, group_num=4, feature_dim=1024)
        self.ac = CLSTokenProcessor().to(device)
    ...
    def encode_state_prompt(self):
        ...
        # self.state_prompt_tokens                      torch.Size([2, 77])
        # self.state_prompt_embedding.repeat(2, 1, 1))  torch.Size([2, 12, 768])
        # state_x                                       torch.Size([2, 77, 768])
        state_x = torch.cat([self.state_prompt_embedding.repeat(2, 1, 1), state_x], dim=1)[:, :77, :]
        # torch.Size([2, 77, 768])
        state_x = state_x + self.positional_embedding.type(self.dtype)  # 位置编码
        state_x = state_x.permute(1, 0, 2)  # NLD -> LND
        # torch.Size([77, 2, 768])
        state_x = self.transformer(state_x)  # 文本编码
        # torch.Size([77, 2, 768])
        state_x = state_x.permute(1, 0, 2)  # LND -> NLD
        state_x = self.ln_final(state_x).type(self.dtype)  # torch.Size([2, 77, 768])
        # self.state_prompt_tokens.argmax(dim=-1)  tensor([4, 4], device='cuda:0')
        # self.prompt_len + self.state_prompt_tokens.argmax(dim=-1)  tensor([16, 16], device='cuda:0')
        state_x = state_x[torch.arange(state_x.shape[0]), self.prompt_len + self.state_prompt_tokens.argmax(dim=-1)] @ self.text_projection
        # 每个文本样本挑选一个特殊的token，比如这里挑选的就是第16个token
        # torch.Size([2, 768])
        return state_x


    def encode_state_prompt_test(self, cls_tokens):
        # cls_tokens: [8, 4, 1024] - 8张图像，每张4个cls token
        batch_size = cls_tokens.shape[0]  # 8
        num_prompts = 2  # 两个文本提示

        # 1. 投影 cls_tokens 到 768 维
        cls_tokens_projected = cls_tokens[:, :, :768] # [8, 4, 768]
        # 2. 复制 cls_tokens 给每个提示：[8, 4, 768] -> [8, 2, 4, 768]
        cls_tokens_expanded = cls_tokens_projected.unsqueeze(1).repeat(1, num_prompts, 1, 1)  # [8, 2, 4, 768]
        # 3. 处理文本 token embeddings
        state_x = self.token_embedding(self.state_prompt_tokens).type(self.dtype)
        # self.state_prompt_tokens: [2, 77]
        # state_x: [2, 77, 768]

        # 4. 扩展 state_x 到 batch_size=8: [2, 77, 768] -> [8, 2, 77, 768]
        state_x = state_x.unsqueeze(0).repeat(batch_size, 1, 1, 1)  # [8, 2, 77, 768]

        # 5. 扩展 state_prompt_embedding: [12, 768] -> [8, 2, 12, 768]
        state_prompt_emb = self.state_prompt_embedding.repeat(2, 1, 1)
        state_prompt_emb = state_prompt_emb.unsqueeze(0).repeat(
            batch_size, 1, 1, 1
        )  # [8, 2, 12, 768]

        # 6. 拼接: 可训练提示(12) + cls_tokens(4) + 固定提示(61)
        # 然后截取前77个
        state_x = torch.cat([
            state_prompt_emb,  # [8, 2, 12, 768] - 可训练的提示token
            cls_tokens_expanded,  # [8, 2, 4, 768]  - 图像cls tokens
            state_x  # [8, 2, 77, 768] - 文本token embeddings
        ], dim=2)[:, :, :77, :]  # [8, 2, 77, 768] 截取前77个

        # 7. reshape 以便后续处理: [8, 2, 77, 768] -> [16, 77, 768]
        state_x = state_x.view(batch_size * num_prompts, 77, 768)  # [16, 77, 768]

        # 8. 位置编码
        state_x = state_x + self.positional_embedding.type(self.dtype)

        # 9. Transformer 处理
        state_x = state_x.permute(1, 0, 2)  # [77, 16, 768]
        state_x = self.transformer(state_x)
        state_x = state_x.permute(1, 0, 2)  # [16, 77, 768]
        state_x = self.ln_final(state_x).type(self.dtype)

        # 10. 提取特定位置的token
        # 现在索引需要加上插入的 cls_tokens 长度(4)
        # self.state_prompt_tokens: [2, 77] -> 需要扩展到 [16, 77]
        state_prompt_tokens_expanded = self.state_prompt_tokens.repeat(batch_size, 1)  # [16, 77]
        token_indices = self.prompt_len + 4 + state_prompt_tokens_expanded.argmax(dim=-1)  # [16]

        state_x = state_x[torch.arange(batch_size * num_prompts), token_indices] @ self.text_projection
        # [16, 768]

        # 11. reshape 回 [8, 2, 768]
        state_x = state_x.view(batch_size, num_prompts, -1)  # [8, 2, 768]
        return state_x

    def encode_state_prompt_test2(self, cls_tokens):
        # cls_tokens: [8, 4, 1024] - 8张图像，每张4个cls token
        batch_size = cls_tokens.shape[0]  # 8
        num_prompts = 2  # 两个文本提示
        ...
        # self.state_prompt_tokens: [2, 77] -> 需要扩展到 [16, 77]
        state_prompt_tokens_expanded = self.state_prompt_tokens.repeat(batch_size, 1)  # [16, 77]
        token_indices = self.prompt_len + state_prompt_tokens_expanded.argmax(dim=-1)  # [16]
        state_x = state_x[torch.arange(batch_size * num_prompts), token_indices] @ self.text_projection
        # [16, 768]
        ...
        cls = self.ac(cls_tokens)
        cls = cls.unsqueeze(1).repeat(1, 2, 1)

        state_x = torch.cat([state_x, cls], dim=2)
        # print(state_x.shape)
        state_x = state_x.reshape(batch_size * 2, -1)
        state_x = self.mlp(state_x)
        state_x = state_x.reshape(batch_size, 2, -1)
        return state_x
    ...
    def encode_image(self, image, feature_layers=None):
        ...
        # return self.visual(image.type(self.dtype), feature_layers, DPAM_layer=20, ffn=ffn)
    ...
    def detect_encode_image(self, image, args):
        # image_size torch.Size([8, 3, 518, 518])
        img_tokens = self.encode_image(image, args.feature_layers)  # 4层 torch.Size([8, 1370, 1024])
        # img_tokens = self.augment(img_tokens)

        cls_tokens = []  # 使用的是clip原始cls token
        for img_token in img_tokens:
            cls_tokens.append(img_token[:, 0, :])
        cls_tokens = torch.stack(cls_tokens, dim=1)

        img_tokens = self.aggerate_neighbors(img_tokens)
        img_tokens = [self.adaptor(img_token) for img_token in img_tokens]

        img_tokens = [self.visual.ln_post(img_token) @ self.visual.proj for img_token in img_tokens]
        # 12 = 4 * 3 层 torch.Size([8, 1370, 768])
        return img_tokens, cls_tokens
    ...
    def detect_forward_seg(self, image, args):
        if args.epoch == 1:
            img_tokens, cls_tokens = self.detect_encode_image(image, args)
            with torch.no_grad():
                # text_features = self.encode_state_prompt()
                text_features = self.encode_state_prompt_test(cls_tokens.detach())
        if args.epoch == 2:
            with torch.no_grad():
                img_tokens, cls_tokens = self.detect_encode_image(image, args)
            # text_features = self.encode_state_prompt()
        ...
        img_tokens, cls_tokens = self.detect_encode_image(image, args)
        text_features = self.encode_state_prompt_test2(cls_tokens)
        # torch.Size([2, 768])
        ...
        for (i, img_token) in enumerate(img_tokens):
            ...
            score = torch.matmul(img_token, text_features.permute(0, 2, 1)) / 0.07
            scores += score

        prob = torch.softmax(scores, dim=-1)  # torch.Size([8, 1370, 2])
        cls_label = prob[:, 0, 1].view(-1)    # [8]  用第 0 个 patch（cls 位）的异常概率当图像级分数
        predict_map = prob[:, 1:, 1]          # [8, 1369]  扔掉 cls，只留 patch 级异常概率
